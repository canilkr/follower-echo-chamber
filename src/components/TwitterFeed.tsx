...
const TwitterFeed: React.FC<TwitterFeedProps> = ({ username, displayName, autoRefresh = false }) => {
  ...
  const loadTweets = async () => {
    try {
      setLoading(true);
      const fetchedTweets = await fetchTweets(username);
      ...
      if (fetchedTweets.length > 0) {
        toast({
          title: "Feed Updated",
          description: `Loaded ${fetchedTweets.length} tweets from ${displayName}`,
        });
      }
    } catch (error) {
      ...
      toast({
        ...
        description: `Unable to fetch live data for ${displayName}. Please check your Twitter API configuration.`,
        variant: "destructive"
      });
    } finally {
      setLoading(false);
      ...
      <Card className="bg-slate-900/50 border-slate-800">
        <CardHeader>
          ...
          <div className="flex items-center gap-4 text-sm text-slate-400">
            ...
            <Badge className={`${isAutoRefresh ? 'bg-green-500/10 text-green-400' : 'bg-slate-500/10 text-slate-400'} border-slate-700`}>
        ...
        {loading && tweets.length === 0 && (
          <Card className="bg-slate-900/50 border-slate-800">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-slate-400">Loading {displayName}'s latest tweets...</p>
            </CardContent>
          </Card>
        )}
